...
def runtime_output_contract(agent: str) -> str:
    ...
    contract += (
        ...
        " If required inputs or selectors are missing, still return the JSON object with empty evidence arrays and precise data_gaps; "
        "do not replace the JSON object with a prose-only clarification request."
    )
...
def qa_task(agent: str, task_profile: str | None = None) -> str:
    ...
    tasks = {
        ...
        ("package-risk-summary", "evidence-check"): "summarize package risk for ecosystem `pypi`, package `urllib3`, version `1.25.11` using only the package-level `oss` PackageVersion exact lookup by `meta.name==\"pypi://urllib3@1.25.11\"`. Do not resolve this repository to an Endor Project, use MCP discovery, or run broad tenant/repository inventory in runtime QA. If exact evidence is unavailable, return UNKNOWN with evidence_queries and data_gaps; do not recommend a new scan as the default next check.",
        ...
        ("dependency-decision-helper", "evidence-check"): "decide whether ecosystem `pypi`, package `urllib3`, version `1.25.11` is acceptable using only the package-level `oss` PackageVersion exact lookup by `meta.name==\"pypi://urllib3@1.25.11\"`. Do not resolve this repository to an Endor Project, use MCP discovery, or run broad tenant/repository inventory in runtime QA. If evidence is unavailable, return a blocked or degraded verdict with data_gaps.",
