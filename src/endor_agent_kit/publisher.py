...
from endor_agent_kit.publication import CodexHostAdapter, HostArtifactPublication
...
from endor_agent_kit.publication.readme import architecture_readme_section
...
_HOST_ARTIFACT_PUBLICATION = HostArtifactPublication({CODEX_HOST: CodexHostAdapter()})
...
def publish_recipe(recipe_path: str | Path, dest: str | Path) -> list[Path]:
    ...
    if CODEX_HOST in recipe.compatible_hosts:
        bundle = _HOST_ARTIFACT_PUBLICATION.publish(CODEX_HOST, recipe_file, recipe, destination)
        written.extend(bundle.written)
        manifest = _write_manifest(destination, recipe, CODEX_HOST, list(bundle.manifest_records))
...
def _architecture_readme_section(recipe: EndorAgentRecipe) -> list[str]:
    return architecture_readme_section(recipe)
