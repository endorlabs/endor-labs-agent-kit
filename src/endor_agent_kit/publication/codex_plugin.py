    ...
    return {
        "hooks": {
            ...
            "PreToolUse": [
                {
                    "matcher": "Bash",
                    "hooks": [command("enforce-agent-api.sh", "PreToolUse")],
                }
            ],
            "PostToolUse": [
                {
...
def _render_codex_agent_toml(prepared: PreparedSourceRecipe, version: str) -> str:
    ...
    body = (
        "Only after a prerequisite is proven missing, or when the user explicitly "
        f"asks for setup help, use `{CODEX_SETUP_SKILL}`. Do not load setup guidance "
        "during a routine workflow with working Endor access.\n\n"
        f"{body}"
    )
    lines = [
...
def _render_codex_setup_agent_toml(prepared_recipes: list[PreparedSourceRecipe], version: str) -> str:
    ...
    instructions = "\n".join([
        ...
        "MCP remains opt-in: every selected agent must use `endorctl agent api --agent-id <canonical-recipe-id>` for Endor CLI API calls; configure Endor MCP only when a selected MCP-capable workflow needs it or the user explicitly asks.",
