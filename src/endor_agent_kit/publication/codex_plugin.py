...
def publish_codex_plugin_package(
    ...
    logo.write_text(_logo_svg(), encoding="utf-8")
    written.append(logo)
    ...
    plugins_readme.write_text(_plugins_readme(), encoding="utf-8")
...
def _render_codex_agent_toml(prepared: PreparedSourceRecipe) -> str:
    ...
    lines = [
        ...
        f"# endor_agent_kit_agent_id = {json.dumps(recipe.id)}",
        f"# endor_agent_kit_agent_name = {json.dumps(_codex_agent_name(recipe.id))}",
        f"# endor_agent_kit_recipe_version = {json.dumps(recipe.version)}",
        f"# endor_agent_kit_source_recipe = {json.dumps(f'source/agents/{recipe.id}/recipe.yaml')}",
        "",
        f"name = {json.dumps(_codex_agent_name(recipe.id))}",
        f"description = {json.dumps(_single_line(recipe.description))}",
    ]
