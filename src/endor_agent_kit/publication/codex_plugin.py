...
def publish_codex_plugin_package(
    ...
    setup_agent.write_text(_render_codex_setup_agent_toml(sorted_recipes), encoding="utf-8")
...
def _render_codex_setup_agent_toml(prepared_recipes: list[PreparedSourceRecipe]) -> str:
    body = _strip_frontmatter(_render_setup_skill(prepared_recipes))
    body += dedent(
        f"""

        ## Codex Host Contract

        This setup custom agent is installed from the Endor Labs Agent Kit Codex
        plugin. Keep setup read-only unless the user explicitly approves local
        package installation or managed Agent Kit file installation. Do not run
        scans, run `endorctl host-check`, dump Endor config files, print secrets,
        or change workflow agents directly. Use `{CODEX_SETUP_SKILL}` as the
        source setup skill when the host exposes skills more reliably than
        custom agents.
        """
    )
    workflow_names = ", ".join(prepared.recipe.id for prepared in prepared_recipes)
    instructions = body + "\n\nBundled workflow agents: " + workflow_names + ".\n"
    lines = [
        "# Generated by Endor Labs Agent Kit. Do not hand-edit installed copies.",
        "# endor_agent_kit_managed = true",
...
def _render_setup_skill(prepared_recipes: list[PreparedSourceRecipe]) -> str:
    ...
    return "\n".join([
        ...
        "",
        "<!-- Generated by Endor Labs Agent Kit. Do not hand-edit installed copies. -->",
        "<!-- endor_agent_kit_managed=true agent_id=endor-agent-kit-setup host=codex -->",
        "",
