...
CODEX_DIRECTORY_SETUP_SKILL_ID = "endor-agent-kit-setup"
...
CODEX_DIRECTORY_ALL_SKILL_IDS = tuple(
    sorted((*CODEX_DIRECTORY_SKILL_IDS, CODEX_DIRECTORY_SETUP_SKILL_ID))
)
...
def publish_codex_directory_plugin_package(
    ...
    setup_dir = package_dir / "skills" / CODEX_DIRECTORY_SETUP_SKILL_ID
    (setup_dir / "agents").mkdir(parents=True)
    setup_skill = setup_dir / "SKILL.md"
    setup_skill.write_text(
        _render_setup_skill(sorted_recipes, version),
        encoding="utf-8",
    )
    written.append(setup_skill)
    setup_metadata = setup_dir / "agents" / "openai.yaml"
    setup_metadata.write_text(_render_setup_openai_yaml(), encoding="utf-8")
    written.append(setup_metadata)
...
def _render_setup_openai_yaml() -> str:
    return json.dumps(
        {
            "interface": {
                ...
                "default_prompt": (
                    "Use $endor-agent-kit-setup to check Endor Agent Kit readiness."
                ),
            },
            "policy": {"allow_implicit_invocation": True},
        },
...
def _render_setup_skill(
    ...
    workflow_ids = ", ".join(
        f"`{prepared.recipe.id}`" for prepared in prepared_recipes
    )
    ...
    return "\n".join(
        [
            "---",
            f"name: {CODEX_DIRECTORY_SETUP_SKILL_ID}",
            ...
            (
                ...
                f"agent_id={CODEX_DIRECTORY_SETUP_SKILL_ID} host=codex-directory "
                f"package={PLUGIN_NAME} version={version} -->"
            ),
            "",
            "# Endor Agent Kit Setup For Codex",
            "",
            (
                f"This public Codex package contains eleven workflow skills plus "
                f"`{CODEX_DIRECTORY_SETUP_SKILL_ID}`. Plugin installation is already complete; "
                "this package does not bundle a custom-agent installer."
            ),
            "",
            f"Bundled workflows: {workflow_ids}.",
            "",
            ...
            "",
            _setup_source(prepared_recipes).rstrip(),
            "",
...
def _codex_directory_manifest(version: str) -> dict[str, object]:
    return {
        ...
        "interface": {
            ...
            "longDescription": (
                "Use eleven source-generated Endor Labs workflows plus a setup "
                "skill to investigate, triage, plan, and remediate application "
                "security and software supply-chain risks from Codex."
            ),
