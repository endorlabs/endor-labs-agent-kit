...
def root_catalog_readme(
    ...
    *,
    ...
    for item in catalog:
        ...
        cursor_sdk_path = (
            f"`cursor-sdk/agents/{_cursor_agent_name(agent_id)}.md`"
            if CODEX_HOST in item.hosts
            else "-"
        )
        portable_path = f"`{PORTABLE_HOST}/{agent_id}/`" if PORTABLE_HOST in item.hosts else "-"
        agent_rows.append(
            f"| {item.name} | {_agent_summary(agent_id)} | {claude_code_path} | {managed_path} | {codex_path} | {gemini_path} | {cursor_path} | {cursor_sdk_path} | {portable_path} |"
        )
    ...
    quick_start_rows = [
        f"| {_agent_icon(item.id)} {item.name} | {_agent_summary(item.id)} | `{_preferred_agent_prompt_name(item)}` | `{_agent_safety(item.id)}` | `{_agent_first_prompt(item.id)}` |"
        for item in catalog
    ]
...
def _agent_summary(agent_id: str) -> str:
    summaries = {
        "ai-sast-remediation": "Triage Endor AI SAST findings, use exploit and remediation context, and open requested change requests",
        ...
        "dependency-reviewer": "Review an exact package decision, package risk, or repository dependencies through one bounded profile",
        "troubleshooting": "Diagnose Endor Labs errors, warnings, scan failures, slow scans, missing integrations, SSO, containers, policy, and reachability issues",
        ...
        "oss-upgrade-investigator": "Analyze Endor platform upgrade impact with VersionUpgrade, CIA, findings, and manifest context",
        "configuration-automation": "Probe GitHub.com onboarding gaps and prescribe Endor scan profiles, toolchains, package integrations, and reachability setup",
        "remediation-planning": "Preview safe dependency remediation options without opening PRs",
...
def _agent_icon(agent_id: str) -> str:
    icons = {
        ...
        "sca-remediation": "🛠️",
        "oss-upgrade-investigator": "⬆️",
        "vulnerability-explainer": "💬",
    }
    return icons.get(agent_id, "🤖")


def _preferred_agent_prompt_name(item: _AgentCatalogEntry) -> str:
    if CODEX_HOST in item.hosts:
        return _cursor_agent_name(item.id)
    if CLAUDE_CODE_HOST in item.hosts:
        return f"@agent-{item.id}"
    return item.id


def _agent_safety(agent_id: str) -> str:
    mutating = {"ai-sast-remediation", "sca-remediation"}
    return "approval-gated mutating" if agent_id in mutating else "read-only"
...
def _agent_first_prompt(agent_id: str) -> str:
    prompts = {
        ...
        "dependency-reviewer": "Review this repository's exact direct dependencies with the repository-review profile. Keep it read-only.",
        "troubleshooting": "Diagnose this Endor issue from redacted error text and read-only tenant evidence. Keep it read-only.",
        "findings-browser": "Show the critical and high reachable findings for namespace <namespace>. Keep it read-only.",
        "malware-responder": "Assess tenant exposure to malware campaign <campaign>. Keep it read-only and report evidence gaps.",
        "configuration-automation": "Explain what evidence you need to assess GitHub onboarding gaps for this repository. Keep it read-only.",
        "remediation-planning": "Preview remediation options for this repository. Do not edit files or open a PR/MR.",
        ...
        "oss-upgrade-investigator": "Show the safest upgrade path for repository <owner>/<repo> package lodash. Keep it read-only.",
        "vulnerability-explainer": "Explain CVE-2021-44228 using verified Endor evidence. Keep it read-only.",
    }
    return prompts.get(agent_id, f"Use the {agent_id} workflow. Keep it within its generated safety contract.")


def _agent_example(agent_id: str) -> str:
    examples = {
        "ai-sast-remediation": "@agent-ai-sast-remediation triage AI SAST findings for this repository",
        "cicd-posture": "@agent-cicd-posture assess CI/CD and supply chain posture for namespace <namespace>",
        "dependency-reviewer": "@agent-dependency-reviewer review this repository's dependency manifests with bounded read-only evidence",
        "troubleshooting": "@agent-troubleshooting diagnose this Endor scan failure from redacted error text and read-only tenant evidence",
        "findings-browser": "@agent-findings-browser show the critical and high reachable findings for namespace <namespace>",
        "malware-responder": "@agent-malware-responder assess tenant exposure to malware campaign <campaign>",
        "oss-upgrade-investigator": "@agent-oss-upgrade-investigator show the safest upgrade path for repository <owner>/<repo> package lodash",
        "configuration-automation": "@agent-configuration-automation probe GitHub org <org> for Endor monitored-branch onboarding gaps and setup prescriptions",
        "remediation-planning": "@agent-remediation-planning preview remediation options for this repository",
...
def _codex_invoke_prompts(catalog: list[_AgentCatalogEntry]) -> list[str]:
    prompts = {
        "ai-sast-remediation": "Use the ai-sast-remediation skill to triage AI SAST findings for this repository.",
        "cicd-posture": "Use the cicd-posture skill to assess CI/CD and supply chain posture for namespace <namespace>.",
        "troubleshooting": "Use the troubleshooting skill to diagnose this Endor issue from redacted error text and read-only tenant evidence.",
        "findings-browser": "Use the findings-browser skill to show the critical and high reachable findings for namespace <namespace>.",
        "malware-responder": "Use the malware-responder skill to assess tenant exposure to malware campaign <campaign>. Keep it read-only.",
        "configuration-automation": "Use the configuration-automation skill to probe GitHub org <org> for Endor monitored-branch onboarding gaps and setup prescriptions.",
