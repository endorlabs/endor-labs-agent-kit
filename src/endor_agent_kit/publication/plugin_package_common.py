...
def plugin_packages_readme() -> str:
    ...
    return "\n".join([
        ...
        "- `vscode/endor-labs-agent-kit/`: cross-Copilot Agent Plugin (Agent Plugins 1.0)",
        "  with `plugin.json` at its root, portable `skills/<id>/SKILL.md` + `mcp.json`",
        "  (top-level `mcpServers` key, `endor-cli-tools` server), and Copilot components",
        "  under `com.github.copilot/` (`agents/<id>.agent.md`, `rules/`). Installs into",
        "  GitHub Copilot in VS Code, the Copilot CLI, and the Copilot app via Git source",
        "  install (Chat: Install Plugin From Source / `copilot plugin install`); no `.vsix`.",
        "",
