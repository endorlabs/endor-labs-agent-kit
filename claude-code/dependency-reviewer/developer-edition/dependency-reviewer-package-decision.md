---
name: dependency-reviewer-package-decision
description: |
...
- Profiles: `package-decision`. Profile bounds workflow; obey stop; full only on request.
...
- Plans: `package-decision`. Exact/ranked evidence first; selected detail only; skipped lanes -> `data_gaps`.
### Evidence Query Recipes

- `decision-package-risk-exact`/package-decision: `check_dependency_for_risks(ecosystem=<ECOSYSTEM>, dependency_name=<PACKAGE_NAME>, version=<VERSION>)`
- `decision-package-version-exact`/package-decision: `endorctl agent api --agent-id dependency-reviewer list -r PackageVersion -n oss --filter 'meta.name=="<PACKAGE_URL_PREFIX>://<PACKAGE_NAME>@<VERSION>"' --field-mask "uuid,meta.name,spec.ecosystem,spec.package_name,spec.release_timestamp" -o json`
- `decision-vulnerability-enrichment`/package-decision: `get_endor_vulnerability(vulnerability_id=<CVE_OR_GHSA>, namespace=<namespace>)`
...
enum: `profile`, `verdict`; list[string]: `conditions`, `alternatives`, `data_gaps`; string: `summary`; list[object]: `evidence_queries`, `policy_evaluations`; object: `policy_context`
