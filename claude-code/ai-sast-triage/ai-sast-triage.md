...
8. Open PRs/MRs only when explicitly requested: prepare the branch, diff, title, and body first; ask for confirmation before pushing or opening a change request. Re-runs update the agent-owned branch when a change request is already open.
   - Include an agent-readable hidden context block in the PR/MR body when you have the values:
     `<!-- endor-agent-kit:ai-sast-context {"finding_uuid":"...","finding_name":"...","namespace":"...","project_uuid":"...","project_name":"...","repo_full_name":"...","file_path":"..."} -->`
   - Include a concise, safe `Exploit Context` section that explains what the reproduction evidence changed about priority and validation without publishing dangerous live-attack instructions or exact exploit payload strings.
   - Include a `Remediation Guidance Used` section that states whether Endor's guidance was applied as-is, adapted, or rejected, and why.
   - Include a human section explaining the standalone exception path. The agent can create an Endor exception policy only after it verifies AppSec approval evidence on the PR/MR.
