...
This is the Claude Code generated agent for `ai-sast-remediation`.

| Reader | First move |
| --- | --- |
| Human operator | Copy the generated subagent into `.claude/agents/` and restart Claude Code if needed. Then use the example prompt below: @agent-ai-sast-remediation triage AI SAST findings for this repository. Do not open a PR until I approve the patch. |
...
Copy `ai-sast-remediation.md` into your target repository's `.claude/agents/` directory,
...
```text
@agent-ai-sast-remediation triage AI SAST findings for this repository. Do not open a PR until I approve the patch.
```
...
- This agent preserves the AI SAST remediation workflow capabilities as a mutating agent.
