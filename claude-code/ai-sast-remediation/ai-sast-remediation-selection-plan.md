---
name: ai-sast-remediation-selection-plan
description: |
...
- Profiles: `selection-plan`. Profile bounds workflow; obey stop; full only on request.
...
- Plans: `selection-plan`. Exact/ranked evidence first; selected detail only; skipped lanes -> `data_gaps`.
### Evidence Query Recipes

- `project-by-git`/selection-plan: `endorctl agent api --agent-id ai-sast-remediation list -r Project -n <namespace> --filter 'spec.git.full_name=="<owner/repo>"' --page-size 2 --field-mask "uuid,meta.name,meta.parent_uuid,spec.git" -o json`
- `ai-sast-selection`/selection-plan: `endorctl agent api --agent-id ai-sast-remediation list -r Finding -n <namespace> --filter 'context.type==CONTEXT_TYPE_MAIN and spec.project_uuid=="<PROJECT_UUID>" and spec.method=="SYSTEM_EVALUATION_METHOD_DEFINITION_AI_SAST"' --field-mask "uuid,context.type,spec.project_uuid,spec.method,spec.level,spec.source_code_version" --list-all -o json`
- `selected-ai-sast-finding`/selection-plan: `endorctl agent api --agent-id ai-sast-remediation get -r Finding -n <namespace> --uuid <FINDING_UUID> -o json`
