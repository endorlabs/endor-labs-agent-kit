...
The Probe Droid artifact uses read-only Endor lookups through `endorctl agent api --agent-id probe-droid`.
...
After a namespace is selected, every scoped `endorctl agent api --agent-id probe-droid` lookup must pass it
