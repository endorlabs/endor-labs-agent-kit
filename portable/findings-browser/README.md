...
This is the portable runtime generated agent bundle for `findings-browser`.

| Reader | First move |
| --- | --- |
| Human operator | Load `agent.md` and `agent.manifest.json` into your runtime and wire only the adapters your policy allows. Then use the example prompt below: Use this agent to analyze repository <repo> with `findings-browser`. |
...
```bash
endor-agent-kit check-install --host portable --agent findings-browser --portable-dir /path/to/runtime/agents/findings-browser
```
