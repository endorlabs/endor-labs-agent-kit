# AI SAST Remediation Portable Agent Bundle
...
This is the portable runtime generated agent bundle for `ai-sast-remediation`.

| Reader | First move |
| --- | --- |
| Human operator | Load `agent.md` and `agent.manifest.json` into your runtime and wire only the adapters your policy allows. Then use the example prompt below: Use this agent to analyze repository <repo> with `ai-sast-remediation`. |
...
```bash
endor-agent-kit check-install --host portable --agent ai-sast-remediation --portable-dir /path/to/runtime/agents/ai-sast-remediation
```
