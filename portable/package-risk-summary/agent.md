...
- Query order: 1. Resolve package coordinate, version, ecosystem, namespace, and optional repository context. 2. Query PackageVersion or package metadata for the named package/version. 3. Query scoped Finding or available Endor risk evidence only for that package when project context is present.
...
- Query order: 1. Resolve namespace and package coordinate first. 2. Query only exact package/version metadata. 3. If repository scope is supplied, query narrow package-linked Finding evidence.
