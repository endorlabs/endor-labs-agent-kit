...
Final output: no raw shell, `endorctl agent api --agent-id package-risk-summary`, `endorctl scan`, `git`, or source-provider inventory adapter command strings in prose, JSON, validation steps, recommendations, or future actions; summarize intent, selectors, and fields.
