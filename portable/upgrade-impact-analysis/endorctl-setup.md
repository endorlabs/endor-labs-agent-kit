...
The Endor Labs Upgrade Impact Analysis artifact uses read-only Endor lookups through `endorctl agent api --agent-id upgrade-impact-analysis`.
...
After a namespace is selected, every scoped `endorctl agent api --agent-id upgrade-impact-analysis` lookup must pass it
