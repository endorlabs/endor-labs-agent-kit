# Endor Labs Upgrade Impact Analysis Portable Agent Bundle
...
This is the portable runtime generated agent bundle for `upgrade-impact-analysis`.

| Reader | First move |
| --- | --- |
| Human operator | Load `agent.md` and `agent.manifest.json` into your runtime and wire only the adapters your policy allows. Then use the example prompt below: Use this agent to analyze repository <repo> with `upgrade-impact-analysis`. |
...
```bash
endor-agent-kit check-install --host portable --agent upgrade-impact-analysis --portable-dir /path/to/runtime/agents/upgrade-impact-analysis
```
