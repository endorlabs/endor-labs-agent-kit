...
This is the portable runtime generated agent bundle for `cicd-posture`.

| Reader | First move |
| --- | --- |
| Human operator | Load `agent.md` and `agent.manifest.json` into your runtime and wire only the adapters your policy allows. Then use the example prompt below: Use this agent to analyze repository <repo> with `cicd-posture`. |
...
```bash
endor-agent-kit check-install --host portable --agent cicd-posture --portable-dir /path/to/runtime/agents/cicd-posture
```
