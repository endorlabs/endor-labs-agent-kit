...
Generated from Endor Agent Kit recipe `dependency-decision-helper` v1.0.0 for portable runtimes.
...
Final output: no raw shell, `endorctl agent api --agent-id dependency-decision-helper`, `endorctl scan`, `git`, or source-provider inventory adapter command strings in prose, JSON, validation steps, recommendations, or future actions; summarize intent, selectors, and fields.
...
MCP tools when they are available. runtime command execution is allowed only for the read-only Endor
