...
The Dependency Decision Helper artifact uses read-only Endor lookups through `endorctl agent api --agent-id dependency-decision-helper`.
...
After a namespace is selected, every scoped `endorctl agent api --agent-id dependency-decision-helper` lookup must pass it
