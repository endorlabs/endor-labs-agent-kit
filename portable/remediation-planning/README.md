# Remediation Planning Portable Agent Bundle

Preview safe remediation options without opening PRs.

## Start Here

This is the portable runtime generated agent bundle for `remediation-planning`.

| Reader | First move |
| --- | --- |
| Human operator | Load `agent.md` and `agent.manifest.json` into your runtime and wire only the adapters your policy allows. Then use the example prompt below: Use this agent to analyze repository <repo> with `remediation-planning`. |
...
```bash
endor-agent-kit check-install --host portable --agent remediation-planning --portable-dir /path/to/runtime/agents/remediation-planning
```
