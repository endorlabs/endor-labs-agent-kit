...
Generated from Endor Agent Kit recipe `endor-troubleshooter` v0.1.0 for portable runtimes.
...
Use documented read-only `endorctl agent api --agent-id endor-troubleshooter list`, `get`, or query commands only.
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource Project --namespace <namespace> \
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource Project --namespace <namespace> --traverse \
  --filter '<name_or_repository_selector_filter>' \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,meta.create_time,meta.update_time,spec" \
  -o json
```
...
```bash
endorctl agent api --agent-id endor-troubleshooter get --resource ScanResult --namespace <namespace> --uuid <scan_result_uuid> \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,meta.create_time,meta.update_time,spec" \
  -o json
```

```bash
endorctl agent api --agent-id endor-troubleshooter list --resource ScanResult --namespace <namespace> \
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource ScanWorkflowResult --namespace <namespace> \
  --filter 'meta.parent_uuid=="<project_uuid>"' \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,meta.create_time,meta.update_time,spec" \
  -o json
```
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource ScanWorkflow --namespace <namespace> \
  --filter 'meta.parent_uuid=="<project_uuid>"' \
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource ScanProfile --namespace <namespace> \
  --filter 'meta.parent_uuid=="<project_uuid>"' \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,spec" \
  -o json
```
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource PackageVersion --namespace <namespace> \
...
```bash
endorctl agent api --agent-id endor-troubleshooter get --resource PackageVersion --namespace <namespace> --uuid <package_version_uuid> \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,meta.create_time,meta.update_time,spec" \
  -o json | jq '{
    uuid,
    name:.meta.name,
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource PackageManager --namespace <namespace> \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,spec" \
  -o json
```
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource SCMCredential --namespace <namespace> \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,spec" \
  -o json
```
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource Installation --namespace <namespace> \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,spec" \
  -o json
```
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource IdentityProvider --namespace <namespace> \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,spec" \
  -o json
```
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource PRCommentConfig --namespace <namespace> \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,spec" \
  -o json
```
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource NotificationTarget --namespace <namespace> \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,meta.create_time,meta.update_time,spec" \
  -o json
```

```bash
endorctl agent api --agent-id endor-troubleshooter list --resource Exporter --namespace <namespace> \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,meta.create_time,meta.update_time,spec" \
  -o json
```
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource Finding --namespace <namespace> \
  --filter 'context.type==CONTEXT_TYPE_MAIN and spec.project_uuid=="<project_uuid>"' \
  --field-mask "uuid,context,meta.name,meta.parent_uuid,meta.tags,spec" \
  -o json
```
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource Policy --namespace <namespace> \
  --field-mask "uuid,meta.name,meta.parent_uuid,meta.tags,spec" \
  -o json
```
...
```bash
endorctl agent api --agent-id endor-troubleshooter list --resource CallGraphData --namespace <namespace> \
  --filter 'meta.parent_uuid=="<project_uuid>"' \
...
`endorctl agent api --agent-id endor-troubleshooter`, `endorctl scan`, `endorctl --version`, `git`, or source-provider inventory adapter command
...
After selecting a namespace, pass it explicitly with `-n <namespace>` or `--namespace <namespace>` for every scoped `endorctl agent api --agent-id endor-troubleshooter` lookup; do not rely on bare `endorctl` namespace resolution. If an Endor MCP call cannot be explicitly scoped to the selected namespace, use it only after proving the active process/config namespace matches the selected namespace. Otherwise use explicit `endorctl agent api --agent-id endor-troubleshooter -n <namespace>` or report a `data_gaps` entry.
...
- Template: `endorctl agent api --agent-id endor-troubleshooter list -r Project -n <namespace> --filter 'spec.git.full_name=="<owner/repo>"' --field-mask "uuid,meta.name,meta.parent_uuid,spec.git" --list-all -o json`
...
- Template: `endorctl agent api --agent-id endor-troubleshooter get -r ScanResult -n <namespace> --uuid <SCAN_RESULT_UUID> -o json`
...
- Template: `endorctl agent api --agent-id endor-troubleshooter get -r Finding -n <namespace> --uuid <FINDING_UUID> -o json`
...
Final output: no raw shell, `endorctl agent api --agent-id endor-troubleshooter`, `endorctl scan`, `git`, or source-provider inventory adapter command strings in prose, JSON, validation steps, recommendations, or future actions; summarize intent, selectors, and fields.
...
Use runtime command execution only for the documented read-only `endorctl agent api --agent-id endor-troubleshooter` lookups in these
