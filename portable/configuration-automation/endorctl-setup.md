...
The Configuration Automation artifact uses read-only Endor lookups through `endorctl agent api --agent-id configuration-automation`.
...
After a namespace is selected, every scoped `endorctl agent api --agent-id configuration-automation` lookup must pass it
