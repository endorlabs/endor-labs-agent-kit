...
Generated from Endor Agent Kit recipe `configuration-automation` v0.1.0 for portable runtimes.
...
In customer-managed runtime, the source-provider inventory adapter may be unavailable even when runtime command execution is available.
...
```bash
endorctl agent api --agent-id configuration-automation list \
  --resource Project \
  <namespace_flag> \
  --list-all \
...
`endorctl agent api --agent-id configuration-automation list`, `endorctl agent api --agent-id configuration-automation get`, the source-provider repository-list adapter, the source-provider repository-detail adapter, or
...
```bash
set -o pipefail; endorctl agent api --agent-id configuration-automation list --resource Installation <namespace_flag> --list-all --field-mask "uuid,meta.name,meta.tags,meta.create_time,meta.update_time,spec" | jq '{count:(.list.objects|length), installations:(.list.objects|map({uuid,name:.meta.name, selected_project_count:((.spec.project_uuids // [])|length), selected_project_uuids:(.spec.project_uuids // []), enabled_features:(.spec.enabled_features // []), invalid:.spec.invalid, sync_errors:(.spec.sync_errors // [])}))}'
```

```bash
set -o pipefail; endorctl agent api --agent-id configuration-automation list --resource ScanProfile <namespace_flag> --list-all --field-mask "uuid,meta.name,meta.tags,spec" | jq '{count:(.list.objects|length), profiles:(.list.objects|map({uuid,name:.meta.name, tags:.meta.tags, spec_keys:(.spec|keys), toolchain_present:(.spec.toolchain_profile? != null or .spec.tool_chain_profile? != null), automated_scan_present:(.spec.automated_scan_parameters? != null)}))[0:20]}'
```

```bash
set -o pipefail; endorctl agent api --agent-id configuration-automation list --resource PackageManager <namespace_flag> --list-all --field-mask "uuid,meta.name,meta.tags,spec" | jq '{count:(.list.objects|length), package_managers:(.list.objects|map({uuid,name:.meta.name, tags:.meta.tags, spec_keys:(.spec|keys), status:(.spec.package_manager_status // null), registry_hosts:([.spec[]? | objects | .url? // .registry_url? // empty] | unique)}))[0:20]}'
```

```bash
set -o pipefail; endorctl agent api --agent-id configuration-automation list --resource PackageVersion <namespace_flag> --list-all --filter 'context.type==CONTEXT_TYPE_MAIN and spec.project_uuid=="<project_uuid>"' --field-mask "uuid,meta.name,context,spec.ecosystem,spec.project_uuid,spec.resolution_errors" | jq 'def has_meaningful_errors: (.spec.resolution_errors | if . == null then false elif type=="object" then ((keys|length) > 0) elif type=="array" then (length > 0) else false end); {count:(.list.objects|length), meaningful_resolution_error_count:([.list.objects[] | select(has_meaningful_errors)] | length), empty_resolution_error_object_count:([.list.objects[] | select((.spec.resolution_errors|type)=="object" and (.spec.resolution_errors|keys|length)==0)] | length), examples:[.list.objects[] | select(has_meaningful_errors) | {package:.meta.name, ecosystem:.spec.ecosystem, project_uuid:.spec.project_uuid, error_keys:(.spec.resolution_errors|keys), best_category:.spec.resolution_errors.resolved.error_analysis_best_match.error_category, status_error:(.spec.resolution_errors.resolved.status_error // .spec.resolution_errors.unresolved.status_error // .spec.resolution_errors.call_graph.status_error // null), rule:.spec.resolution_errors.resolved.rule}][0:10]}'
```
...
After selecting a namespace, pass it explicitly with `-n <namespace>` or `--namespace <namespace>` for every scoped `endorctl agent api --agent-id configuration-automation` lookup; do not rely on bare `endorctl` namespace resolution. If an Endor MCP call cannot be explicitly scoped to the selected namespace, use it only after proving the active process/config namespace matches the selected namespace. Otherwise use explicit `endorctl agent api --agent-id configuration-automation -n <namespace>` or report a `data_gaps` entry.
...
Select single_repo, selected_repositories, or fleet scope before tools, then classify every in-scope repository as healthy, unhealthy, missing, ambiguous, excluded, or precisely unknown. Batch complete inventory by resource and group unhealthy repositories by failure signature before targeted detail. github_app_coverage must be a non-empty object, never null. Every repository lane row, including ambiguous_matches, must include repository or repo_full_name and default_branch; use UNKNOWN plus data_gaps when branch proof is unavailable, not github_repository alone. Onboarded rows must include project_uuid and endor_monitored_branch; if monitored branch proof is unavailable, put the row in onboarded_repositories_with_gaps, not onboarded_healthy_repositories. Keep coverage counts separate from health quality counts, and surface compact top offenders before JSON drill-down. Treat Project.spec.monitored_branch as optional; use normalized ScanResult.spec.refs fallback or UNKNOWN plus data_gaps for branch proof. Successful current scans are authoritative evidence that optional setup metadata is not itself a blocker.
- Use when: The user asks what configuration or errors prevent successful Endor scans for one repository, a selected set, or a fleet. A read-only host check needs onboarding evidence without full org analysis.
- Minimal evidence: Namespace provenance, explicit scope_mode, repository denominator, Project matches, latest main-context ScanResult health, and package-resolution error cohorts with traversal only when child namespaces are in scope.
- Stop when: Every in-scope repository is classified healthy, actionable, missing, excluded, or precisely unknown. Do not inspect ScanProfile, PackageManager, Installation, or repository files unless a named failure cohort requires it.
- Output focus: Return coverage_summary, grouped issue_cohorts, compact repository classifications, recommended_actions, validation_plan, inventory_artifacts, evidence_queries, and data_gaps.
...
#### `evidence-check` - Adaptive Scan Readiness Query Plan

Explain what prevents 100 percent successful monitored-branch scans across one repository, a selected set, or a fleet.
- Query order: 1. Select scope_mode before tools: single_repo for one repository, selected_repositories for 2 to 100 explicit repositories, and fleet for organization, namespace-wide, all-repository, or 100-percent-success requests. 2. Establish the complete in-scope Project denominator, then collect complete main-context ScanResult and PackageVersion evidence through one artifact-backed call per resource. 3. Use deterministic configuration projections, classify latest scan health, and group shared failure signatures before any representative ScanProfile, PackageManager, Installation, or repository-file detail.
- Avoid: Do not enumerate unrelated organizations or child namespaces outside the explicit scope. Do not multiply a full diagnostic workflow by repository count or treat missing optional metadata as a failure. Do not run scans or update repo settings.
- Stop after: Stop when every in-scope repository is healthy, actionable, missing, excluded, or precisely unknown and each issue cohort has one evidence-backed human action plus validation step.
- Data gaps: Record incomplete denominators, artifact truncation, missing GitHub permissions, unresolved Project matches, unknown branches, and failure cohorts whose minimal configuration evidence remains unavailable in data_gaps.
...
#### `configuration-projects-complete` (evidence-check)

- Canonical: `configuration-projects-complete`
- Resource: `Project`
- Purpose: Establish the complete Project denominator for a selected set or fleet.
- Selection condition: `runtime.configuration_scope_inventory_required`
- Result delivery: `runtime.large_result_artifact_required`
- Template: `endorctl agent api --agent-id configuration-automation list -r Project -n <namespace> <namespace_traversal> <PROJECT_SCOPE_FILTER_ARG> --field-mask "uuid,meta.name,meta.parent_uuid,spec.git" --list-all -o json`
- Fields: `uuid`, `meta.name`, `meta.parent_uuid`, `spec.git`
- Constraints: Use normalized selectors, the matching configuration project projection, and explicit traversal only when children are in scope.

#### `configuration-scans-complete` (evidence-check)

- Canonical: `configuration-scans-complete`
- Resource: `ScanResult`
- Purpose: Reduce complete main-context scan history to latest Project health and failure cohorts.
- Selection condition: `runtime.configuration_scope_inventory_required`
- Result delivery: `runtime.large_result_artifact_required`
- Template: `endorctl agent api --agent-id configuration-automation list -r ScanResult -n <namespace> <namespace_traversal> --filter '<SCAN_SCOPE_FILTER>' --field-mask "uuid,meta.parent_uuid,meta.create_time,meta.update_time,context.type,spec.status,spec.type,spec.exit_code,spec.refs,spec.stats" --list-all -o json`
...
- Constraints: Use current-run Project scope plus main context and the configuration-scans projection.

#### `configuration-packages-complete` (evidence-check)

- Canonical: `configuration-packages-complete`
- Resource: `PackageVersion`
- Purpose: Reduce complete main-context package inventory to resolution and reachability cohorts.
- Selection condition: `runtime.configuration_scope_inventory_required`
- Result delivery: `runtime.large_result_artifact_required`
- Template: `endorctl agent api --agent-id configuration-automation list -r PackageVersion -n <namespace> <namespace_traversal> --filter '<PACKAGE_SCOPE_FILTER>' --field-mask "uuid,meta.name,context.type,spec.project_uuid,spec.resolution_errors" --list-all -o json`
- Fields: `uuid`, `meta.name`, `context.type`, `spec.project_uuid`, `spec.resolution_errors`
- Constraints: Use current-run Project scope plus main context and the configuration-packages projection; empty resolution_errors is healthy.
...
- Preferred evidence resources: `Project`, `ScanProfile`, `PackageManager`, `PackageVersion`, `ScanResult`.
...
`endorctl agent api --agent-id configuration-automation list`, `endorctl agent api --agent-id configuration-automation get`, and `endorctl --version`. Do not run
...
- `issue_cohorts` (`list[object]`): Distinct scan, dependency-resolution, reachability, onboarding, or configuration failure signatures with affected counts, capped examples, recommended actions, and validation steps.
- `inventory_artifacts` (`list[object]`): Protected complete-inventory artifact references with resource, scope, row count, SHA-256, byte size, projection, and truncation status; never raw rows.
...
Final output: no raw shell, `endorctl agent api --agent-id configuration-automation`, `endorctl scan`, `git`, or source-provider inventory adapter command strings in prose, JSON, validation steps, recommendations, or future actions; summarize intent, selectors, and fields.
