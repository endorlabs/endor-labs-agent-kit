...
Final output: no raw shell, `endorctl agent api --agent-id remediation-planner`, `endorctl scan`, `git`, or source-provider inventory adapter command strings in prose, JSON, validation steps, recommendations, or future actions; summarize intent, selectors, and fields.
...
Use runtime command execution only for read-only `endorctl agent api --agent-id remediation-planner` lookups. Do not edit files, open pull requests, create policies, or mutate Endor state.
