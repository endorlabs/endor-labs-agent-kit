...
- `dependency_graph_audit` (object, required): Bounded Maven or Gradle audit of graph manipulations on the selected dependency path, including forced overrides, exclusions, substitutions, exact replacements, evidence status, and required graph/runtime validation.
