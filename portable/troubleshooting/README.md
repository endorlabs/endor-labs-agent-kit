# Troubleshooting Portable Agent Bundle
...
This is the portable runtime generated agent bundle for `troubleshooting`.

| Reader | First move |
| --- | --- |
| Human operator | Load `agent.md` and `agent.manifest.json` into your runtime and wire only the adapters your policy allows. Then use the example prompt below: Use this agent to analyze repository <repo> with `troubleshooting`. |
...
```bash
endor-agent-kit check-install --host portable --agent troubleshooting --portable-dir /path/to/runtime/agents/troubleshooting
```
