...
Generated from Endor Agent Kit recipe `oss-upgrade-investigator` v1.0.0 for portable runtimes.
...
In the runtime, read the repository root and `origin` remote URL, then resolve the matching
...
After selecting a namespace, pass it explicitly with `-n <namespace>` or `--namespace <namespace>` for every scoped `endorctl agent api --agent-id oss-upgrade-investigator` lookup; do not rely on bare `endorctl` namespace resolution. If an Endor MCP call cannot be explicitly scoped to the selected namespace, use it only after proving the active process/config namespace matches the selected namespace. Otherwise use explicit `endorctl agent api --agent-id oss-upgrade-investigator -n <namespace>` or report a `data_gaps` entry.
...
```bash
endorctl agent api --agent-id oss-upgrade-investigator list \
  --resource VersionUpgrade \
  <namespace_flag> \
...
Final output: no raw shell, `endorctl agent api --agent-id oss-upgrade-investigator`, `endorctl scan`, `git`, or source-provider inventory adapter command strings in prose, JSON, validation steps, recommendations, or future actions; summarize intent, selectors, and fields.
