# OSS Upgrade Investigator Portable Agent Bundle
...
This is the portable runtime generated agent bundle for `oss-upgrade-investigator`.

| Reader | First move |
| --- | --- |
| Human operator | Load `agent.md` and `agent.manifest.json` into your runtime and wire only the adapters your policy allows. Then use the example prompt below: Use this agent to analyze repository <repo> with `oss-upgrade-investigator`. |
...
```bash
endor-agent-kit check-install --host portable --agent oss-upgrade-investigator --portable-dir /path/to/runtime/agents/oss-upgrade-investigator
```
