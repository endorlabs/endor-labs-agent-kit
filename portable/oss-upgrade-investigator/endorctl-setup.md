...
The OSS Upgrade Investigator artifact uses read-only Endor lookups through `endorctl agent api --agent-id oss-upgrade-investigator`.
...
After a namespace is selected, every scoped `endorctl agent api --agent-id oss-upgrade-investigator` lookup must pass it
