...
| `plugins/` | Generated plugin packages for Claude Code, Codex CLI, the universal Plugins Directory shared by ChatGPT and Codex, Gemini CLI, and Antigravity CLI. |
...
- `src/endor_agent_kit/publication/codex_directory_plugin.py`
...
The universal-directory skills-only archive is intentionally not generated
during normal catalog publication. Validate the unpacked package with
`scripts/build_codex_directory_submission.py validate --root .`; only the
review-gated `ai-plugins` workflow builds the deterministic ZIP from an exact
immutable mirror SHA.
