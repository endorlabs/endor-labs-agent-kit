...
The Endor Troubleshooter artifact uses read-only Endor lookups through `endorctl agent api --agent-id endor-troubleshooter`.
...
After a namespace is selected, every scoped `endorctl agent api --agent-id endor-troubleshooter` lookup must pass it
