...
Generated from Endor Agent Kit recipe `ai-sast-remediation` v0.1.0 for Codex.
...
11. Create the Endor exception policy only after verified AppSec approval, a stable match key, and duplicate prevention: render the proposed policy spec first, including current finding UUID as evidence, `exception_match`, project scope, reason, expiration, approver, approval evidence URL, and policy name. Before asking for confirmation or calling `endorctl agent api --agent-id ai-sast-remediation`, list existing Endor Policy resources in the namespace and check for an active matching exception policy by generated policy name, stable match fingerprint, project UUID, and exception reason. Do not use Finding UUID as the policy matcher; Finding UUIDs can change on rescans.
   ...
   - If no matching policy exists, ask for explicit confirmation in the Codex session before calling `endorctl agent api --agent-id ai-sast-remediation` to create the policy.
