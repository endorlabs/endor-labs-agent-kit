...
Generated from Endor Agent Kit recipe `repository-dependency-reviewer` v1.0.0 for Endor Labs Agent Kit Gemini CLI extension.
...
Use only Gemini CLI read-only file tools: `Glob`, `Grep`, `LS`, and `Read`.
Do not use Bash.
...
Use only Endor MCP tools and Gemini CLI read-only file tools. Do not use Bash
