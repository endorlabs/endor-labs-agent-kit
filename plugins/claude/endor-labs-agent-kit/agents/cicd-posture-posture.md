...
<!-- endor_agent_kit_managed=true agent_id=cicd-posture profile_id=posture host=claude-code-plugin -->
...
- Profiles: `posture`. Profile bounds workflow; obey stop; full only on request.
### Evidence Query Plans

- Plans: `posture`. Exact/ranked evidence first; selected detail only; skipped lanes -> `data_gaps`.
...
- `github-branch-protection`/posture: `GitHub REST GET /repos/<owner>/<repo>/branches/<default_branch>/protection`
- `github-workflow-files`/posture: `GitHub REST GET /repos/<owner>/<repo>/contents/.github/workflows?ref=<default_branch>`
