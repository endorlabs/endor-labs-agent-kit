...
<!-- endor_agent_kit_managed=true agent_id=findings-browser profile_id=browse host=claude-code-plugin -->
...
- Profiles: `browse`. Profile bounds workflow; obey stop; full only on request.
### Evidence Query Plans

- Plans: `browse`. Exact/ranked evidence first; selected detail only; skipped lanes -> `data_gaps`.
...
`findings_verdict`, `summary`, `applied_filters`, `severity_summary`, `finding_results`, `pagination`, `evidence_queries`, `data_gaps`, `policy_context`, `policy_evaluations`
