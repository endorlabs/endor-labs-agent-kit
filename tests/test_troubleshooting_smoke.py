...
def _copy_agent(tmp_path: Path) -> Path:
    src = repo_root() / "source" / "agents" / "troubleshooting"
    dst = tmp_path / "troubleshooting"
...
def test_troubleshooting_recipe_is_read_only_and_mcp_free(tmp_path):
    recipe = _copy_agent(tmp_path)
    data = yaml.safe_load(recipe.read_text(encoding="utf-8"))

    assert validate_recipe_file(recipe) == []
    assert data["id"] == "troubleshooting"
...
def test_troubleshooting_compiled_artifact_carries_diagnostic_contract(tmp_path):
    recipe = _copy_agent(tmp_path)
    ...
    assert "Troubleshooting Evidence Contract" in artifact
    ...
    assert "Do not put raw\n`endorctl agent api --agent-id troubleshooting`, `endorctl scan`, `endorctl --version`, `git`, or `gh` command" in artifact
    ...
    assert "`endorctl`, `endorctl agent api --agent-id troubleshooting\nlist`, `git`, `gh`" in artifact
...
def test_troubleshooting_managed_agents_artifacts_are_read_only(tmp_path):
    ...
    assert managed["name"] == "Troubleshooting"
    assert managed["model"] == "claude-sonnet-4-6"
    assert managed["metadata"]["endor_agent_kit_recipe_id"] == "troubleshooting"
    ...
    assert "read-only `endorctl agent api --agent-id troubleshooting` lookups" in managed["system"]
...
def test_troubleshooting_publish_writes_host_catalog_surfaces(tmp_path):
    ...
    assert written_paths == {
        "claude-code/troubleshooting/troubleshooting.md",
        "claude-code/troubleshooting/troubleshooting-diagnose.md",
        "claude-code/troubleshooting/README.md",
        "claude-code/troubleshooting/architecture.svg",
        "claude-code/troubleshooting/endorctl-setup.md",
        "claude-managed-agents/troubleshooting/agent.yaml",
        "claude-managed-agents/troubleshooting/environment.yaml",
        "claude-managed-agents/troubleshooting/session-template.yaml",
        "claude-managed-agents/troubleshooting/README.md",
        "claude-managed-agents/troubleshooting/architecture.svg",
        "claude-managed-agents/troubleshooting/endorctl-setup.md",
        "codex/troubleshooting/SKILL.md",
        "codex/troubleshooting/README.md",
        "codex/troubleshooting/architecture.svg",
        "codex/troubleshooting/endorctl-setup.md",
        "gemini/troubleshooting/SKILL.md",
        "gemini/troubleshooting/troubleshooting.md",
        "gemini/troubleshooting/README.md",
        "gemini/troubleshooting/architecture.svg",
        "gemini/troubleshooting/endorctl-setup.md",
        "portable/troubleshooting/README.md",
        "portable/troubleshooting/agent.md",
        "portable/troubleshooting/agent.manifest.json",
    ...
    gemini_dir = dest / "gemini" / "troubleshooting"
    portable_dir = dest / "portable" / "troubleshooting"
    assert_host_bundle_files(
        agent_dir,
        {"troubleshooting.md", "README.md", "architecture.svg", "endorctl-setup.md"},
    )
    assert_host_bundle_files(
        managed_dir,
    ...
    assert_host_bundle_files(
        gemini_dir,
        {"SKILL.md", "troubleshooting.md", "README.md", "architecture.svg", "endorctl-setup.md"},
    )
    assert_host_bundle_files(
        portable_dir,
    ...
    assert "Troubleshooting" in root_readme
    assert "Diagnose Endor Labs errors, warnings, scan failures" in root_readme
    assert "claude-code/troubleshooting/" in root_readme
    assert "claude-managed-agents/troubleshooting/" in root_readme
    assert "codex/troubleshooting/" in root_readme
    assert "gemini/troubleshooting/" in root_readme
    assert "portable/troubleshooting/" in root_readme
    assert "@agent-troubleshooting diagnose this Endor scan failure from redacted error text" in root_readme
    assert "@agent-troubleshooting diagnose this Endor scan failure" in agent_readme
    assert "large monorepo" in agent_readme
    assert "Diagnose this Endor scan failure" in managed_readme
    assert "Troubleshooting Codex Skill" in codex_readme
    assert "Use the troubleshooting skill to diagnose this Endor scan failure" in codex_readme
    assert "## Codex Host Contract" in codex_skill
    assert "future_action_contracts" in codex_skill
    assert "Troubleshooting Gemini CLI Bundle" in gemini_readme
    assert "## Gemini CLI Host Contract" in gemini_skill
    assert "future_action_contracts" in gemini_skill
    assert "Troubleshooting uses only read-only Endor lookups" in setup
...
def test_troubleshooting_eval_cases_cover_troubleshooting_lanes():
    evals = yaml.safe_load(
        (repo_root() / "source" / "agents" / "troubleshooting" / "evals" / "cases.yaml").read_text()
    )
